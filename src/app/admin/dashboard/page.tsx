...
export default function AdminDashboardPage() {
    ...
    if (success) {
      setStudents(prev => prev.map(s => {
        if (s.id === studentId) {
          ...
          return { ...s, subscriptions: newSubs }
        }
        return s
      }))
    }
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto animate-in fade-in duration-500 bg-background min-h-screen text-foreground">
      {/* Header */}
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-muted/50 text-muted-foreground text-xs uppercase font-bold tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4">Suscripciones Activas / Inactivas</th>
                    ...
                    <tr key={student.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 rounded-full bg-primary/20 text-primary flex items-center justify-center font-bold shadow-inner">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-2 max-w-[400px]">
                          {courses.map(course => {
                            const isSub = student.subscriptions.find(s => s.courseId === course.id)?.subscribed || false
                            return (
                              <button
                                key={course.id}
                                onClick={() => toggleSubscription(student.id, course.id, isSub)}
                                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-sm ${
                                  isSub 
                                    ? "bg-[#22c55e]/15 border-[#22c55e]/40 text-[#22c55e] hover:bg-destructive/20 hover:border-destructive/40 hover:text-destructive group" 
                                    : "bg-muted/80 border-border/80 text-muted-foreground hover:bg-[#22c55e]/20 hover:border-[#22c55e]/40 hover:text-[#22c55e] group"
                                }`}
                              >
                                {isSub ? <CheckCircle2 className="h-3.5 w-3.5 group-hover:hidden" /> : <div className="h-3.5 w-3.5 rounded-full border border-current group-hover:hidden" />}
                                {isSub ? <XCircle className="h-3.5 w-3.5 hidden group-hover:block" /> : <CheckCircle2 className="h-3.5 w-3.5 hidden group-hover:block" />}
                      ...
                      <td className="px-6 py-4 text-right">
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-accent/10 text-accent border border-accent/20">
                          Activo
                        </span>
                      </td>
                    </tr>
