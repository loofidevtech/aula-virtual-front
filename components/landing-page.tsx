    ...
    <div className="relative overflow-hidden bg-background">
          ...
          <div className="grid gap-16 lg:grid-cols-2 lg:items-center">
            <div className="space-y-12">
              ...
              <div className="space-y-6">
                <h1 className="text-balance text-6xl font-black tracking-tighter text-foreground md:text-7xl lg:text-8xl leading-[0.95] animate-in fade-in slide-in-from-left duration-1000">
                ...
                <p className="max-w-xl text-pretty text-2xl text-muted-foreground leading-relaxed font-medium animate-in fade-in slide-in-from-left duration-1000 delay-200">
                  <span className="font-black text-foreground">LoofiDev Academy</span> es la plataforma educativa diseñada para maximizar tu potencial académico con tecnología de vanguardia.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-6 pt-4 animate-in fade-in slide-in-from-bottom duration-1000 delay-400">
                ...
                <Button 
                  size="lg" 
                  variant="outline" 
                  className="h-20 px-12 rounded-3xl border-4 border-muted font-black text-xl text-foreground hover:bg-muted hover:text-white transition-all active:scale-95"
                >
                  Conocer Planes
                </Button>
              </div>
              ...
              <div className="flex items-center gap-8 pt-8 animate-in fade-in duration-1000 delay-600">
                <div className="flex -space-x-4">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="h-14 w-14 rounded-full border-4 border-background bg-muted flex items-center justify-center font-black text-accent">
                      {String.fromCharCode(64 + i)}
                    </div>
                  ))}
                </div>
                <p className="text-lg font-bold text-muted-foreground">
                  <span className="text-foreground font-black">+500 alumnos</span> ya confían en nosotros
                </p>
              </div>
            </div>
            ...
            <div className="relative animate-in fade-in zoom-in duration-1000 delay-300">
              <div className="relative aspect-square w-full">
                ...
                <div className="group relative h-full w-full overflow-hidden rounded-[4rem] border-[12px] border-card bg-card shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] transition-transform duration-700 hover:rotate-2">
                  <Image 
                    src="/placeholder.jpg" 
                    alt="Academia Online" 
                    fill 
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <button className="group/play relative flex h-28 w-28 items-center justify-center rounded-full bg-primary text-white shadow-2xl transition-all hover:scale-110 hover:bg-accent hover:text-background active:scale-95">
                      <PlayCircle className="h-16 w-16 fill-current" />
                      <span className="absolute -inset-4 animate-ping rounded-full bg-primary/30 group-hover/play:bg-accent/30" />
                    </button>
                  </div>
      ...
      <section className="bg-muted/30 py-32 overflow-hidden relative">
        <div className="container mx-auto px-4 mb-20 relative z-10 text-center space-y-6">
          <h2 className="text-5xl md:text-6xl font-black text-foreground tracking-tight">Testimonios que <span className="text-primary italic">Inspiran</span></h2>
        ...
        <div className="flex w-[200%] animate-infinite-scroll">
          {[...testimonials, ...testimonials].map((t, i) => (
            <div key={i} className="w-[350px] md:w-[500px] px-8 flex-shrink-0">
              <Card className="bg-card border border-border/50 text-card-foreground rounded-[4rem] h-full hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.5)] transition-all duration-500 hover:-translate-y-4 p-4">
                <CardContent className="p-10 space-y-8">
                  ...
                  <p className="italic text-2xl text-muted-foreground leading-relaxed font-medium">"{t.text}"</p>
                  <div className="flex items-center gap-6 border-t border-border pt-8">
                    <div className="h-16 w-16 rounded-[1.5rem] bg-muted flex items-center justify-center font-black text-accent text-2xl shadow-inner">
                       {t.name.charAt(0)}
                    </div>
                    <div>
                      <p className="font-black text-foreground text-2xl tracking-tight">{t.name}</p>
      ...
      {/* Methodology Section */}
      <section className="bg-background px-4 py-32 md:py-48">
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid gap-12 md:grid-cols-3">
            {methodologyCards.map((card, index) => (
              <Card
                key={index}
                className="group border border-border/50 bg-card/50 rounded-[4rem] p-8 shadow-none hover:shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] hover:bg-card transition-all duration-700 hover:-translate-y-6"
              >
                <CardHeader className="p-8">
                  ...
                  <CardTitle className="text-3xl font-black text-card-foreground tracking-tight">{card.title}</CardTitle>
                </CardHeader>
      ...
      {/* CTA Section */}
      <section className="px-4 py-40 relative bg-background overflow-hidden border-t border-border/50">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(var(--accent)_1px,transparent_1px)] [background-size:40px_40px] opacity-10" />
        </div>

        <div className="container mx-auto max-w-5xl text-center space-y-16 relative z-10">
          <h2 className="text-6xl md:text-8xl font-black tracking-tighter text-foreground leading-[0.9]">
