...
export function LockModal({ isOpen, onClose, courseName }: LockModalProps) {
  ...
  const message = encodeURIComponent(`Hola Albert Math Academy! 👋 Deseo obtener acceso completo al curso de ${courseName}. Me gustaría recibir más información sobre los costos y el proceso de inscripción.`)
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-[3rem] bg-white shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="relative p-8 md:p-12 text-center">
          ...
          <h2 className="text-3xl font-black text-secondary mb-4 leading-tight">
            ¡Has descubierto contenido <span className="text-primary italic">Premium!</span>
          </h2>

          <p className="text-lg text-muted-foreground font-medium mb-8 leading-relaxed">
            Has alcanzado el límite gratuito de 3 videos para <span className="font-bold text-secondary">{courseName}</span>. 
            Para acceder a todo nuestro repertorio de clases, simulacros y material especializado, contáctanos ahora.
          </p>
          ...
          <div className="mt-8 pt-8 border-t border-secondary/5 flex items-center justify-center gap-6">
            <div className="flex -space-x-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-10 w-10 rounded-full border-2 border-white bg-secondary/10 flex items-center justify-center font-bold text-xs text-secondary">
                  {String.fromCharCode(64 + i)}
                </div>
              ))}
            </div>
            <p className="text-xs font-bold text-secondary/60">
              Más de <span className="text-secondary font-black">500 alumnos</span> ya son premium
            </p>
          </div>
        </div>
      </div>
